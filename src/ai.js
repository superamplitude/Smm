...
async function recommendServices({ userId, goal, budget, network }) {
  ...
  const output = result.ok ? result.data : { recommendations: [], strategy: 'IA não configurada.' };
...
async function operationalBrief() {
  ...
  const input = { orders, payments, recent };
  const system = `Você é a IA gestora operacional de um SaaS de marketing digital. Analise os indicadores, identifique gargalos, riscos e oportunidades. Não autorize movimentações financeiras. Responda JSON com: health em good|attention|critical, summary string, alerts array, recommended_actions array.`;
  const result = await callAI({ system, input });
  const output = result.ok ? result.data : { health: 'attention', summary: 'IA não configurada.', alerts: [], recommended_actions: [] };
  await logDecision({ type: 'operational_brief', input, output, status: result.ok ? 'completed' : 'fallback' });
  return output;
}

module.exports = { callAI, analyzeOrder, recommendServices, operationalBrief };
